...
void Main()
{
    ...
    using (var scope = LinqPadRhetosHost.CreateScope(rhetosHostAssemblyPath))
    {
		...
		var book = repository.Bookstore.Book.Query().First();
		repository.Bookstore.Chapter.Insert(new Bookstore.Chapter{ BookID=book.ID,Heading="Prvo poglavlje"});
		repository.Bookstore.Chapter.Insert(new Bookstore.Chapter{ BookID=book.ID,Heading="Drugo poglavlje"});
		
		repository.Bookstore.BookInfo.Load().Dump(1);
